...
echo "echo force-confold | tee -a  /etc/dpkg/dpkg.cfg" >> /tmp/do.sh
echo "echo force-confdef | tee -a  /etc/dpkg/dpkg.cfg" >> /tmp/do.sh
echo "apt-get update -y --force-yes" >> /tmp/do.sh
echo "apt-get install  -y --force-yes  apache2 htop nano" >> /tmp/do.sh
echo "sudo apt-get -y --force-yes upgrade" >> /tmp/do.sh
echo "sudo apt-get -y --force-yes dist-upgrade" >> /tmp/do.sh
